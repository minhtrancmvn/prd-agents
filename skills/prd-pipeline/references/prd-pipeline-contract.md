...
A terminal response must not claim document write success unless target exists and author handoff confirms success. Generated artifacts remain outside repository source and are reported by absolute path. A pre-QA blocked or failed run has contiguous artifacts through failed phase and `07-summary`; it has no later phases, QA, or Phase 6 artifacts. A blocking QA terminal (a QA attempt that did not reach `CHECKLIST_PASSED`) ends with `07-summary` and likewise carries no Phase 6 artifact. Successful terminal response requires all base phases, QA, and — only when the final QA verdict is `CHECKLIST_PASSED` — a valid Phase 6 artifact: `06-repair-skipped`, `06-consolidation-attempt-1`, or the prior `06-repair-attempt-N` pairs produced by the run's repair cycle. A clean passing verdict with no prior repair attempt requires `06-repair-skipped` (or `06-consolidation-attempt-1` when consolidation ran).
